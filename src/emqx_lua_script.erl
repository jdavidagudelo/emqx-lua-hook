...
on_client_subscribe_single(ClientId, _Username, TopicItem = {Topic, Opts}, LuaState) ->
    ?LOG(debug, "hook client(~s/~s) will subscribe: ~p~n", [_Username, ClientId, Topic]),
    case catch luerl:call_function([on_client_subscribe], [ClientId, _Username, Topic], LuaState) of
        {[false], _St} ->
...
on_session_subscribed(#{}, TopicItem = {<<$$, _Rest/binary>>, _Opts}, _ScriptName, _LuaState) ->
    %% ignore topics starting with $
    {ok, TopicItem};
on_session_subscribed(#{client_id := ClientId, username := UserName}, TopicItem = {Topic, _Opts}, _ScriptName, LuaState) ->
...
on_session_unsubscribed(#{}, TopicItem = {<<$$, _Rest/binary>>, _Opts}, _ScriptName, _LuaState) ->
    %% ignore topics starting with $
    {ok, TopicItem};
on_session_unsubscribed(#{client_id := ClientId, username := UserName}, TopicItem = {Topic, _Opts}, _ScriptName, LuaState) ->
